...
Though in the present-day atmospheric CO2 concentrations are [rising precipitously](https://keelingcurve.ucsd.edu/), geologic processes like rock weathering have, over billion-year geologic timescales, led to substantial reductions in atmospheric CO<sub>2</sub>. At the same time, the emergence of oxygenic photosynthesis led to a build up of O<sub>2</sub> in Earth's atmosphere, which is problematic for organisms relying on rubisco to fix carbon. In this short primer, [Patrick Shih](http://shih-lab.ucdavis.edu/) and I reflect on how these two trends might have conspired to promote the evolution of novel photosynthetic physiologies (several types of CO<sub>2</sub> concentrating mechanisms) that cope with low levels of the growth substrate (CO<sub>2</sub>) and the inhibitor (O<sub>2</sub>). We describe how these strategies differ between photosynthetic bacteria, algae and land plants and suggest that these differences might not be coincidental (i.e. no "frozen accident"). 
