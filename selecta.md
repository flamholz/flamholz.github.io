...
All plants, algae and cyanobacteria rely on the Calvin-Benson-Bassham (CBB) cycle for growth. Rubisco is the central enzyme of the CBB cycle and the most abundant enzyme on the planet. It does the tricky bit where CO<sub>2</sub> gets "fixed." Not only is Rubisco responsible for the vast majority of carbon fixation on Earth, it is also one of the best-characterized enzymes. While it is often claimed that Rubisco is slow, in actuality Rubisco is an average enzyme in terms of "turnover number" (maximum rate per active site). In practice, the true speed of Rubisco carboxylation is much slower than measured because it reacts non-specifically with molecular oxygen (O<sub>2</sub>). I certainly think it's surprising that Rubisco is not faster or more specific given how important and abundant it is. 

People often argue that there are tradeoffs between speed and specificity - i.e. that Rubisco can't be both fast and specific at the same time. Various correlations between Rubisco’s kinetic parameters are taken to support this argument since they  suggest that faster rubiscos (higher turnover numbers) are less CO<sub>2</sub>-specific. But these analyses were based on relatively little data (about 20 Rubiscos) because relatively little high-quality data was available. We collected a dataset containing high-quality measurements of about 300 Rubiscos to re-examine correlations between kinetic parameters. Most of the interesting correlations are now substantially weakened, which induced us to re-evaluate the mechanistic chemical models that were argued to force tradeoffs between Rubisco kinetic parameters in previous works.

The main takeaway from examining our much-larger dataset is that Rubisco kinetic parameters display surprisingly little variation. Turnover numbers for most enzymes vary over 5-10 fold (measured by a multiplicative standard deviation) but Rubisco turnover numbers vary less than twofold. Rubisco CO<sub>2</sub>-specificity varies even less than that - about 30% among the Form I enzymes (the kind found in plants and cyanobacteria). When we consider Rubiscos from particular kinds of species (e.g. C<sub>3</sub> or C<sub>4</sub> plants, cyanos) we find even less variation in CO<sub>2</sub>-specificity. We think that extremely narrow variation suggests tight chemical constraints on Rubisco evolution and offer a coarse-grained model of Rubisco catalysis that might help understand the constraints imposed on the evolution of the most central enzyme in the carbon cycle. I think this work should cause us to re-evaluate prospects for Rubisco engineering, since a tightly-constrained enzyme is probably hard to engineer even. We put a fair bit of effort into suggesting experiments and engineering strategies that could test our model, so I do hope that someone will try to prove us wrong. 
