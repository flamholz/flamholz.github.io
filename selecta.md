...
All plants, algae and cyanobacteria rely on the Calvin-Benson-Bassham (CBB) cycle for growth. Rubisco is the central enzyme of the CBB cycle - it does the bit where CO2 gets fixed. Not only is Rubisco responsible for the vast majority of carbon fixation on Earth, it is also one of the best-characterized enzymes. While it is often claimed that Rubisco is slow, in actuality Rubisco is a roughly average enzyme in terms of speed. In practice, however, the true speed of Rubisco carboxylation is slower than measured because it reacts non-specifically with molecular oxygen. At first blush it is surprising that Rubisco is not faster or more specific given its centrality and abundance. 

It is often argued that there are tradeoffs between speed and specificity - i.e. that Rubisco can't be both fast and specific at the same time and so nature must choose one or the other. Various correlations between Rubisco’s kinetic parameters are taken to support this argument since they appear to suggest that fast rubiscos are less specific and, conversely, specific rubiscos are relatively slow. But these analysis were historically based on relatively little data (Rubiscos from ≈20 organisms) because relatively little data was available. In this manuscript we re-examine correlations between Rubisco kinetic parameters with an expanded dataset including data from >200 organisms. Most correlations are substantially weaker than before and only one model appears to survive in a 10x larger dataset. In this model, increased catalytic efficiency (kcat/KM) for carboxylation requires a similar increase in catalytic efficiency for the competing oxygenation reaction. This model is supported evidenced by a strong power-law correlation between the two catalytic efficiencies. We think that these results imply that Rubisco evolution is constrained mostly by the physicochemical limits of O2/CO2 recognition, which might mean that it will be very challenging to engineer Rubisco to be "better."

This work started off as a journal club of [Tcherkez et al. PNAS 2006](http://www.pnas.org/content/103/19/7246) and [Savir et al. PNAS 2010](http://www.pnas.org/content/107/8/3475.short) that [Dave Savage](http://www.savagelab.org/) suggested I run when we visited [Ron Milo's lab](https://www.weizmann.ac.il/plants/Milo/home) in January 2018. [Noam Prywes](https://twitter.com/noamprywes?lang=en) and I scrounged together data for the JC and Uri Moran did a lot of the heavy lifting, combing the literature for as much Rubisco data as we could find. Posted to [bioRxiv](https://www.biorxiv.org/content/early/2018/11/14/470021) November 2018. 
