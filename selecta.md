...
#### *Preprint* "[Revisiting tradeoffs between Rubisco kinetic parameters](https://www.biorxiv.org/content/10.1101/470021v3)" 

![RuBisCO illustration by David Goodsell](https://cdn.rcsb.org/pdb101/motm/images/two-forms.gif){: .center-image }

All plants, algae and cyanobacteria rely on the Calvin-Benson-Bassham (CBB) cycle for growth. Rubisco is the central enzyme of the CBB cycle and probably the most abundant enzyme on the planet. It does the tricky bit where CO2 gets "fixed". Not only is Rubisco responsible for the vast majority of carbon fixation on Earth, it is also one of the best-characterized enzymes. While it is often claimed that Rubisco is slow, in actuality Rubisco is a roughly average enzyme in terms of "turnover number" (maximum speed per active site). In practice, the true speed of Rubisco carboxylation is much slower than measured because it reacts non-specifically with molecular oxygen (O2). I certainly think it's surprising that Rubisco is not faster or more specific given how important and abundant it is. 

People often argue that there are tradeoffs between speed and specificity - i.e. that Rubisco can't be both fast and specific at the same time. Various correlations between Rubisco’s kinetic parameters are taken to support this argument since they appear to suggest that fast rubiscos are less specific. But these analysis were historically based on relatively little data (like 20 Rubiscos) because relatively little data was available. We re-examine correlations between Rubisco kinetic parameters with a dataset of >200 Rubiscos. Most correlations are weaker than before and only one model appears to survive in a 10x larger dataset. Under this model, increased catalytic efficiency (kcat/KM) for carboxylation requires a similar increase in catalytic efficiency for oxygenation. 

We think that these results imply that Rubisco evolution is constrained mostly by the physicochemical limits of O2/CO2 recognition. To put this another way: it seems that there is strong coupling between the initial steps of carboxylation and oxygenation, suggesting it would not be productive to target these initial steps for improvement. At the same time, the fact that correlations between kinetic parameters are weakened overall might suggest that the subsequent steps (hydration of the intermediate and cleavage of a C-C bond) could be improved (to some degree) without negatively affecting the initial steps of CO2/O2 discrimination. I think this work should cause us to re-evaluate prospects for Rubisco engineering. On the one hand, some tradeoffs constrain Rubisco evolution rather stringently. On the other hand, key catalytic parameters (like kcat and KC) are less correlated than previously thought, which could mean that it is possible to improve the  rate of Rubisco carboxylation without sacrificing too much in the way of specificity. 
